...
/**
 ...
 This option is only applicable to ``BharatMapsRasterTileSource`` objects; it is ignored
 when initializing ``BharatMapsVectorTileSource`` objects.
 */
FOUNDATION_EXTERN BharatMaps_EXPORT const BharatMapsTileSourceOption BharatMapsTileSourceOptionTileSize;

/**
 ``BharatMapsRasterTileSource`` is a map content source that supplies raster image tiles
 ...
 ```swift
 let source = BharatMapsRasterTileSource(identifier: "clouds", tileURLTemplates:
 ...
 #### Related examples
 TODO: Add raster imagery, learn how to add a ``BharatMapsRasterStyleLayer``
 to your map using an ``BharatMapsRasterTileSource``.
 */
BharatMaps_EXPORT
@interface BharatMapsRasterTileSource : BharatMapsTileSource
...
/**
 ...
 @param tileSize The width and height (measured in points) of each tiled image
    in the raster tile source. See the ``BharatMapsTileSourceOptionTileSize``
