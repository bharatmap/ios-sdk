...
/**
 An ``BharatMapsPolygon`` object represents a closed shape consisting of four or more
 ...
 You can add polygon shapes to the map by adding them to an ``BharatMapsShapeSource``
 object. Configure the appearance of an ``BharatMapsShapeSource``’s or
 ``BharatMapsVectorTileSource``’s polygons collectively using an ``BharatMapsVectorTileSource`` or
 ``BharatMapsSymbolStyleLayer`` object. To access a polygon’s attributes, use an
 ``BharatMapsPolygonFeature`` object.

 Alternatively, you can add a polygon overlay directly to a map view using the
 ``BharatMapsMapView/addAnnotation:`` or ``BharatMapsMapView/addOverlay:`` method. Configure
 a polygon overlay’s appearance using
 ``BharatMapsMapViewDelegate/mapView:strokeColorForShapeAnnotation:`` and
 ``BharatMapsMapViewDelegate/mapView:fillColorForPolygonAnnotation:``.
 ...
 ``BharatMapsPolyline`` object.

 A polygon may have one or more interior polygons, or holes, that you specify as
 ``BharatMapsPolygon`` objects with the `+polygonWithCoordinates:count:interiorPolygons:`
 ...
 an ``BharatMapsMultiPolygon`` or ``BharatMapsMultiPolygon`` object.
 ...
 ``BharatMapsPolygon`` object from an array of coordinates.
 */
BharatMaps_EXPORT
@interface BharatMapsPolygon : BharatMapsMultiPoint <BharatMapsOverlay>
 ...
 */
@property (nonatomic, nullable, readonly) NSArray<BharatMapsPolygon *> *interiorPolygons;

/**
 Creates and returns an ``BharatMapsPolygon`` object from the specified set of
 coordinates.
...
/**
 ...
 @param coords The array of coordinates defining the shape. The data in this
    array is copied to the new object.
 @param count The number of items in the `coords` array.
 @param interiorPolygons An array of ``BharatMapsPolygon`` objects that define regions
...
+ (instancetype)polygonWithCoordinates:(const CLLocationCoordinate2D *)coords
                                 count:(NSUInteger)count
                      interiorPolygons:(nullable NSArray<BharatMapsPolygon *> *)interiorPolygons;

@end

/**
 An ``BharatMapsMultiPolygon`` object represents a shape consisting of one or more
 ...
 atoll: the inner island would be one ``BharatMapsPolygon`` object, while the surrounding
 ...
 ``BharatMapsShapeSource`` object. Configure the appearance of an ``BharatMapsShapeSource``’s or
 ``BharatMapsVectorTileSource``’s multipolygons collectively using an ``BharatMapsVectorTileSource``
 or ``BharatMapsSymbolStyleLayer`` object.

 You cannot add an ``BharatMapsMultiPolygon`` object directly to a map view using
 ``BharatMapsMapView/addAnnotation:`` or ``BharatMapsMapView/addOverlay:``. However, you can
 add the `polygons` array’s items as overlays individually.
 */
BharatMaps_EXPORT
@interface BharatMapsMultiPolygon : BharatMapsShape <BharatMapsOverlay>

/**
 An array of polygons forming the multipolygon.
 */
@property (nonatomic, copy, readonly) NSArray<BharatMapsPolygon *> *polygons;
 ...
 */
+ (instancetype)multiPolygonWithPolygons:(NSArray<BharatMapsPolygon *> *)polygons;
