...
/**
 ...
 let triangle = BharatMapsPolygon(coordinates: &coordinates, count: UInt(coordinates.count))
 let region = BharatMapsShapeOfflineRegion(styleURL: BharatMapsStyle.lightStyleURL, shape: triangle,
 fromZoomLevel: 11, toZoomLevel: 14) let context = "Triangle Region".data(using: .utf8)
 BharatMapsOfflineStorage.shared.addPack(for: region, withContext: context!)
 ```

 This class requires fewer resources than BharatMapsTilePyramidOfflineRegion
 for irregularly shaped regions.
 */
BharatMaps_EXPORT
@interface BharatMapsShapeOfflineRegion : NSObject <BharatMapsOfflineRegion, NSSecureCoding, NSCopying>

/**
 The shape for the geographic region covered by the downloaded
 tiles.
 */
@property (nonatomic, readonly) BharatMapsShape *shape;

/**
 The minimum zoom level for which to download tiles and other resources.

 For more information about zoom levels, ``BharatMapsMapView/zoomLevel``.
 */
...
/**
 ...
 This is the designated initializer for ``BharatMapsShapeOfflineRegion``.
