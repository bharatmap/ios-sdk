...
/**
 An ``BharatMapsPointCollection`` object represents a shape consisting of one or more
 ...
 You can add point collections to the map by adding them to an ``BharatMapsShapeSource``
 object. Configure the appearance of an ``BharatMapsShapeSource``’s or
 ``BharatMapsVectorTileSource``’s point collections collectively using an
 ``BharatMapsCircleStyleLayer`` or ``BharatMapsCircleStyleLayer`` object. To access a point
 collection’s attributes, use an ``BharatMapsPointCollectionFeature`` object.

 You cannot add an ``BharatMapsPointCollection`` object directly to a map view as an
 annotation. However, you can create individual ``BharatMapsPointAnnotation`` objects
 from the `coordinates` array and add those annotation objects to the map view
 using the ``BharatMapsMapView/addAnnotations:`` method.

 A point collection is known as a
 <a href="https://tools.ietf.org/html/rfc7946#section-3.1.3">MultiPoint</a>
 geometry in GeoJSON. Do not confuse ``BharatMapsPointCollection`` with ``BharatMapsPointCollection``,
 the abstract superclass of ``BharatMapsPolyline`` and ``BharatMapsPolyline``.
 */
BharatMaps_EXPORT
@interface BharatMapsPointCollection : BharatMapsShape <BharatMapsOverlay>

/**
 Creates and returns a ``BharatMapsPointCollection`` object from the specified set of
 coordinates.
