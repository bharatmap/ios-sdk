...
FOUNDATION_EXTERN BharatMaps_EXPORT BharatMapsExceptionName const BharatMapsInvalidOfflinePackException;

/**
 The state an offline pack is currently in.
 */
typedef NS_ENUM(NSInteger, BharatMapsOfflinePackState) {
  /**
   ...
   the time the shared ``BharatMapsOfflineStorage`` object sends the first
   ``BharatMapsOfflinePackProgressChangedNotification`` about the pack. For inactive
   packs, you must explicitly request a progress update using the
   ``BharatMapsOfflinePack/requestProgress`` method.

   An invalid pack always has a state of ``BharatMapsOfflinePackStateInvalid``, never
   ``BharatMapsOfflinePackState/BharatMapsOfflinePackStateUnknown``.
   */
  BharatMapsOfflinePackStateUnknown = 0,
  /**
   The pack is incomplete and is not currently downloading.

   This is the initial state of a pack that is created using the
   ``BharatMapsOfflineStorage/addPackForRegion:withContext:completionHandler:``
   method, as well as after the ``BharatMapsOfflinePack/suspend`` method is
   called.
   */
  BharatMapsOfflinePackStateInactive = 1,
  /**
   The pack is incomplete and is currently downloading.

   This is the state of a pack after the ``BharatMapsOfflinePack/resume`` method is
   called.
   */
  BharatMapsOfflinePackStateActive = 2,
  /**
   The pack has downloaded to completion.
   */
  BharatMapsOfflinePackStateComplete = 3,
  /**
   The pack has been removed using the
   ``BharatMapsOfflineStorage/removePack:withCompletionHandler:`` method. Sending
   any message to the pack will raise an exception.
   */
  BharatMapsOfflinePackStateInvalid = 4,
};
...
/**
 ...
 progress.
 */
typedef struct __attribute__((objc_boxable)) BharatMapsOfflinePackProgress {
  /**
...
} BharatMapsOfflinePackProgress;

/**
 An ``BharatMapsOfflinePack`` represents a collection of resources necessary for viewing
 a region offline to a local database.

 To create an instance of ``BharatMapsOfflinePack``, use the
 ``BharatMapsOfflineStorage/addPackForRegion:withContext:completionHandler:`` method.
 A pack created using `BharatMapsOfflinePack/init` is immediately invalid.

 ### Example
 ```swift
 BharatMapsOfflineStorage.shared.addPack(for: region, withContext: context) { (pack, error) in
 ...
 \(#file)(\(#line)) in \(#function)") return
     ...
     // Start an BharatMapsOfflinePack download
     pack.resume()
 }
 ```
 */
BharatMaps_EXPORT
@interface BharatMapsOfflinePack : NSObject

/**
 The region for which the pack manages resources.
 */
@property (nonatomic, readonly) id<BharatMapsOfflineRegion> region;
...
/**
 ...
 ``BharatMapsOfflinePackState/BharatMapsOfflinePackStateUnknown`` by default. To request the pack’s status, use
 the
 ...
 ``BharatMapsOfflinePackProgressChangedNotification``s about this pack that come from the
 default notification center.
 */
@property (nonatomic, readonly) BharatMapsOfflinePackState state;
...
/**
 ...
 default notification center.
 */
@property (nonatomic, readonly) BharatMapsOfflinePackProgress progress;
...
/**
 ...
 set to ``BharatMapsOfflinePackState/BharatMapsOfflinePackStateUnknown``, observe KVO change notifications on this
 pack’s `state` key path, then call this method. Alternatively, you can add an
 observer for ``BharatMapsOfflinePackProgressChangedNotification`` about this pack that
