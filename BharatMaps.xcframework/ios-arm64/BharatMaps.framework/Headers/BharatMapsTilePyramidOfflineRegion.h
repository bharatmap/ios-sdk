...
/**
 ...
 use the BharatMapsShapeOfflineRegion class instead.

 ### Example
 ```swift
 let northeast = CLLocationCoordinate2D(latitude: 40.989329, longitude: -102.062592)
 let southwest = CLLocationCoordinate2D(latitude: 36.986207, longitude: -109.049896)
 let bbox = BharatMapsCoordinateBounds(sw: southwest, ne: northeast)

 let region = BharatMapsTilePyramidOfflineRegion(styleURL: BharatMapsStyle.lightStyleURL, bounds: bbox,
 ...
 */
BharatMaps_EXPORT
@interface BharatMapsTilePyramidOfflineRegion : NSObject <BharatMapsOfflineRegion, NSSecureCoding, NSCopying>

/**
 The coordinate bounds for the geographic region covered by the downloaded
 tiles.
 */
@property (nonatomic, readonly) BharatMapsCoordinateBounds bounds;
...
/**
 ...
 This is the designated initializer for ``BharatMapsTilePyramidOfflineRegion``.
...
- (instancetype)initWithStyleURL:(nullable NSURL *)styleURL
                          bounds:(BharatMapsCoordinateBounds)bounds
