...
@protocol BharatMapsCalloutViewDelegate;
...
/**
 ...
 To receive updates from an object that conforms to the ``BharatMapsCalloutView`` protocol,
 use the optional methods available in the ``BharatMapsCalloutViewDelegate`` protocol.

 #### Related examples
 TODO: Display custom views as callouts by customizing an
 ``BharatMapsCalloutView``.
 */
@protocol BharatMapsCalloutView <NSObject>

/**
 An object conforming to the ``BharatMapsAnnotation`` protocol whose details this callout
 view displays.
 */
@property (nonatomic, strong) id<BharatMapsAnnotation> representedObject;
...
/**
 An object conforming to the ``BharatMapsCalloutViewDelegate`` method that receives
 messages related to the callout view’s interactive subviews.
 */
@property (nonatomic, weak) id<BharatMapsCalloutViewDelegate> delegate;
...
/**
 ...
 ``BharatMapsCalloutView/presentCalloutFromRect:inView:constrainedToRect:animated:``
...
/**
 The ``BharatMapsCalloutViewDelegate`` protocol defines a set of optional methods that
 you can use to receive messages from an object that conforms to the
 ``BharatMapsCalloutView`` protocol. The callout view uses these methods to inform the
 delegate that the user has interacted with the the callout view.
 */
@protocol BharatMapsCalloutViewDelegate <NSObject>
 ...
 */
- (BOOL)calloutViewShouldHighlight:(UIView<BharatMapsCalloutView> *)calloutView;

/**
 Tells the delegate that the callout view has been tapped.
 */
- (void)calloutViewTapped:(UIView<BharatMapsCalloutView> *)calloutView;

/**
 Called before the callout view appears on screen, or before the appearance
 animation will start.
 */
- (void)calloutViewWillAppear:(UIView<BharatMapsCalloutView> *)calloutView;

/**
 Called after the callout view appears on screen, or after the appearance
 animation is complete.
 */
- (void)calloutViewDidAppear:(UIView<BharatMapsCalloutView> *)calloutView;
