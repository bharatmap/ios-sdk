...
@protocol BharatMapsLocationManagerDelegate;

/**
 The ``BharatMapsLocationManager`` protocol defines a set of methods that a class must
 implement in order to serve as the location manager of an ``BharatMapsMapView``. A location
 ...
 To receive location updates from an object that conforms to the ``BharatMapsLocationManager``
 protocol, use the optional methods available in the ``BharatMapsLocationManagerDelegate`` protocol.
 */
@protocol BharatMapsLocationManager <NSObject>
...
/**
 ...
 The default value of this property is `kCLDistanceFilterNone` when ``BharatMapsMapView`` uses its
...
/**
 ...
 The default value is `kCLLocationAccuracyBest` when ``BharatMapsMapView`` uses its
 default location manager.
...
/**
 ...
 ``BharatMapsLocationManager/desiredAccuracy`` property to any value. If the value is
 `CLAccuracyAuthorizationReducedAccuracy`, setting ``BharatMapsLocationManager/desiredAccuracy`` to a value
...
/**
 ...
 The default value is `CLActivityTypeOther` when ``BharatMapsMapView`` uses its
 default location manager.
...
/**
 ...
 Do not set the location manager’s delegate yourself. ``BharatMapsMapView`` sets this property
 after the location manager becomes ``BharatMapsMapView``’s location manager.
 */
@property (nonatomic, weak) id<BharatMapsLocationManagerDelegate> delegate;
...
/**
 The ``BharatMapsLocationManagerDelegate`` protocol defines a set of methods that respond
 to location updates from an ``BharatMapsLocationManager`` object that is serving as the
 location manager of an ``BharatMapsMapView``.
 */
@protocol BharatMapsLocationManagerDelegate <NSObject>
...
/**
 ...
 contains multiple `CLLocation` objects when ``BharatMapsMapView`` uses  its
 default location manager.
 */
- (void)locationManager:(id<BharatMapsLocationManager>)manager
 ...
 */
- (void)locationManager:(id<BharatMapsLocationManager>)manager didUpdateHeading:(CLHeading *)newHeading;
 ...
 */
- (BOOL)locationManagerShouldDisplayHeadingCalibration:(id<BharatMapsLocationManager>)manager;
 ...
 */
- (void)locationManager:(id<BharatMapsLocationManager>)manager didFailWithError:(nonnull NSError *)error;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 140000
/**
 ...
 */
- (void)locationManagerDidChangeAuthorization:(id<BharatMapsLocationManager>)manager;
#endif
