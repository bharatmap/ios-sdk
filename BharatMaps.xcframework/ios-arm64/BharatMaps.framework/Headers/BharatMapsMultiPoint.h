...
/**
 The ``BharatMapsMultiPoint`` class is an abstract superclass used to define shapes
 composed of multiple vertices.

 Create instances of ``BharatMapsPolyline`` or ``BharatMapsPolyline`` in order to use
 properties of ``BharatMapsMultiPoint``. Do not create instances of ``BharatMapsMultiPoint``
 ...
 Do not confuse ``BharatMapsMultiPoint`` with ``BharatMapsMultiPoint``, which represents a
 collection of related but disconnected points.
 */
BharatMaps_EXPORT
@interface BharatMapsMultiPoint : BharatMapsShape
...
/**
 ...
 immediately. If the shape is part of an ``BharatMapsShapeSource`` object, you must
 explicitly set the ``BharatMapsShapeSource/shape`` property in order for any style
