...
typedef NS_ENUM(NSUInteger, BharatMapsAnnotationViewDragState) {
  /**
   The view is not involved in a drag operation.
   */
  BharatMapsAnnotationViewDragStateNone = 0,
  /**
   ...
   */
  BharatMapsAnnotationViewDragStateEnding,
};

/**
 The ``BharatMapsAnnotationView`` class is responsible for marking a point annotation
 with a view. Annotation views represent an annotation object, which is an
 object that corresponds to the ``BharatMapsAnnotation`` protocol. When an annotation’s
 ...
 interactivity such as dragging, you can use an ``BharatMapsAnnotationImage`` instead to
 conserve memory and optimize drawing performance.
 */
BharatMaps_EXPORT
@interface BharatMapsAnnotationView : UIView <NSSecureCoding>
...
/**
 ...
 ``BharatMapsMapView/dequeueReusableAnnotationViewWithIdentifier:`` method.
...
/**
 ...
 with the new view and, in custom subclasses of ``BharatMapsAnnotationView``, customize the view
 ...
 ``BharatMapsMapViewDelegate/mapView:viewForAnnotation:`` method.
 ...
 @return The initialized annotation view object.
 */
- (instancetype)initWithAnnotation:(nullable id<BharatMapsAnnotation>)annotation
 ...
 */
@property (nonatomic, nullable) id<BharatMapsAnnotation> annotation;
...
/**
 ...
 viewpoint). This is also the behavior of ``BharatMapsAnnotationImage`` objects. When the
...
/**
 ...
 view rotates. This is also the behavior of ``BharatMapsAnnotationImage`` objects. When the
...
/**
 ...
 ``BharatMapsAnnotationView`` or ``BharatMapsAnnotationImage`` object, deselects any currently
 selected view.
 ...
 ``BharatMapsAnnotationView/setSelected:animated:`` method instead.
 */
...
/**
 ...
 #### Related examples
 TODO: Enable users to drag ``BharatMapsAnnotationView`` objects on your map.
 */
...
/**
 ...
 All states are handled automatically when the ``BharatMapsAnnotationView/draggable`` property is set to
 `YES`. To perform a custom animation in response to a change to this property,
 override the ``BharatMapsAnnotationView/setDragState:animated:`` method.
 */
@property (nonatomic, readonly) BharatMapsAnnotationViewDragState dragState;
 ...
 */
- (void)setDragState:(BharatMapsAnnotationViewDragState)dragState
