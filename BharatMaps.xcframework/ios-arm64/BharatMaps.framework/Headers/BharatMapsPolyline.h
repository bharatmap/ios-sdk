...
/**
 An ``BharatMapsPolyline`` object represents a shape consisting of two or more vertices,
 ...
 You can add polyline shapes to the map by adding them to an ``BharatMapsShapeSource``
 object. Configure the appearance of an ``BharatMapsShapeSource``’s or
 ``BharatMapsVectorTileSource``’s polylines collectively using an ``BharatMapsLineStyleLayer`` or
 ``BharatMapsSymbolStyleLayer`` object. To access a polyline’s attributes, use an
 ``BharatMapsPolylineFeature`` object.

 Alternatively, you can add a polyline overlay directly to a map view using the
 ``BharatMapsMapView/addAnnotation:`` or ``BharatMapsMapView/addOverlay:`` methods. Configure
 a polyline overlay’s appearance using
 ``BharatMapsMapViewDelegate/mapView:strokeColorForShapeAnnotation:`` and
 ``BharatMapsMapViewDelegate/mapView:lineWidthForPolylineAnnotation:``.
 ...
 ``BharatMapsPolygon`` object. To group multiple polylines together in one shape, use an
 ``BharatMapsMultiPolyline`` or ``BharatMapsShapeCollection`` object.
 ...
 let polyline = BharatMapsPolyline(coordinates: coordinates, count: UInt(coordinates.count))
 ```
 ...
 */
BharatMaps_EXPORT
@interface BharatMapsPolyline : BharatMapsMultiPoint <BharatMapsOverlay>

/**
 Creates and returns an ``BharatMapsPolyline`` object from the specified set of
 coordinates.
...
/**
 An ``BharatMapsMultiPolyline`` object represents a shape consisting of one or more
 ...
 reservation): each carriageway would be a distinct ``BharatMapsPolyline`` object.
 ...
 ``BharatMapsVectorTileSource``’s multipolylines collectively using an
 ``BharatMapsLineStyleLayer`` or ``BharatMapsSymbolStyleLayer`` object.

 You cannot add an ``BharatMapsMultiPolyline`` object directly to a map view using
 ...
 */
BharatMaps_EXPORT
@interface BharatMapsMultiPolyline : BharatMapsShape <BharatMapsOverlay>

/**
 An array of polygons forming the multipolyline.
 */
@property (nonatomic, copy, readonly) NSArray<BharatMapsPolyline *> *polylines;
 ...
 */
+ (instancetype)multiPolylineWithPolylines:(NSArray<BharatMapsPolyline *> *)polylines;
