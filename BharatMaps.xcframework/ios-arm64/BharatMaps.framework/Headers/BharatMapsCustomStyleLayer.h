...
} BharatMapsStyleLayerDrawingContext;
...
///
/// You can access an existing ``BharatMapsCustomStyleLayer`` using the
/// ``BharatMapsStyle/layerWithIdentifier:`` method if you know its identifier;
/// otherwise, find it using the ``BharatMapsStyle/layers`` property. You can also
/// create a new ``BharatMapsCustomStyleLayer`` and add it to the style using a method such as
/// ``BharatMapsStyle/addLayer:``.
///
/// - Warning: This API experimental. It may change
///   at any time without notice.
BharatMaps_EXPORT
@interface BharatMapsCustomStyleLayer : BharatMapsStyleLayer

/// The style that currently contains the layer.
///
/// If the layer is not currently part of any style, this property is `nil`.
@property (nonatomic, weak, readonly) BharatMapsStyle *style;
...
/// Returns an ``BharatMapsCustomStyleLayer`` style layer object initialized with the given identifier.
///
/// After initializing and configuring the style layer, add it to a map view’s style
/// using the ``BharatMapsStyle/addLayer:`` or
...
- (void)willMoveFromMapView:(BharatMapsMapView *)mapView;
...
- (void)drawInMapView:(BharatMapsMapView *)mapView withContext:(BharatMapsStyleLayerDrawingContext)context;
