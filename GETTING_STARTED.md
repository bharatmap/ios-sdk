...
```swift
mapView.setHouseNumbersAutoRefreshEnabled(true) // Default; no application polling needed.
mapView.refreshHouseNumbers() // Optional manual catch-up while active.
```
...
### Active-route congestion (1.0.46)

The app owns traffic fetching/matching. The SDK owns rendering the resulting
congestion on the active navigation line. No routing/search/reverse endpoint is
changed, no route is requested, and guidance does not restart.

```swift
guard let route = mapView.activeNavigationRoute else { return }
// Match your traffic data to THIS snapshot, in travel order.
let levels: [String] = trafficLevels(for: route.locations)
let accepted = mapView.updateNavigationRouteCongestion(
    levels: levels, sessionId: route.sessionId, revision: route.revision
)
// For async traffic work, retain the original sessionId/revision with the result.
// Never relabel an old result with the latest route's identity.
```

`levels[i]` colors the segment from `route.locations[i]` to
`route.locations[i + 1]`. Supply exactly `locations.count - 1` entries, including
entries for duplicate/zero-length coordinate pairs. Values are case-sensitive:

| Level | Default color |
| --- | --- |
| `unknown`, `low` | Current `accentColor`, or the ordinary SDK route color |
| `moderate` | `#F9A825` |
| `heavy` | `#EF6C00` |
| `severe` | `#C62828` |

An overload accepts `palette: [String: UIColor]?`. Only `moderate`, `heavy` and
`severe` may be overridden; missing keys use the defaults above. Colors must
be finite, RGB-convertible UIColors (not pattern colors). Each update replaces
both levels and palette, rather than merging with a previous update. Low and
unknown always use the normal route color; route width and border are unchanged.

```swift
let accepted = mapView.updateNavigationRouteCongestion(
    levels: levels, sessionId: route.sessionId, revision: route.revision,
    palette: ["moderate": .orange, "heavy": .red, "severe": .brown]
)
let cleared = mapView.clearNavigationRouteCongestion(
    sessionId: route.sessionId, revision: route.revision
)
// Passing levels: nil is equivalent to clearing. An empty array is not clear.
```

Call on the main thread. These methods return synchronously: `true` means the
state was accepted (rendering follows the native renderer's next frame, not a
GPU completion callback). `false` means no mutation: wrong session/revision,
inactive/arrived navigation, malformed levels/count/palette, or a non-main-thread
call. If the style is loading, accepted state is retained until style readiness.
No completion/delegate callback is generated by the styling operation.

Vanishing navigation retains the original segment indices and only clips the
traversed prefix, including a partially traversed first segment. Colors are not
redistributed across the remaining geometry. Updates modify the existing source
without recreating it per tick. Theme/style reload retains both coloring and
the rendered route suffix. Camera, padding, follow, puck, progress/rings,
instructions, voice and reroute state are unaffected by applying/clearing colors.

Every reroute/route revision clears the prior levels and palette, even when the
coordinate count is unchanged. Observe `didChangeActiveRoute`, rematch traffic
to the new snapshot, then submit with its identity. Stop also clears styling;
late results cannot recreate an old route. This contract affects only the
SDK-owned active line, not preview routes, app-owned polylines or traffic tiles.
